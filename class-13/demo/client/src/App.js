...
export default class App extends Component {
  ...
  updateCat = async (updatedCatObj, id) => {
    try {
      const updatedCat = await axios.put(url + '/cats/'+ id, updatedCatObj);
      const updatedCatState = this.state.cats.map(cat => {
        if (cat._id === id) {
          return updatedCat.data;
        } 
        return cat;
      })
      this.setState({ cats: updatedCatState });
    } catch (e) {
      console.error(e)
    }
  }
  ...
  render() {
    return (
      <Container>
        ...
        {this.state.cats.length > 0 && <Cats updateCat={this.updateCat} deleteCat={this.deleteCat} cats={this.state.cats} />}
